...
// current folder base URL (e.g., /serviceconnect/)
...
// update
$upd = $conn->prepare("UPDATE orders SET status='Accepted', updated_at=NOW() WHERE id=? AND technician_id=? AND status='Pending'");
$upd->bind_param("ii", $order_id, $tech_id);
$upd->execute();
$ok = $upd->affected_rows > 0;
$upd->close();

header("Location: {$BASE}booking_request.php?" . ($ok ? "ok=accepted" : "error=update_failed"));
exit;
