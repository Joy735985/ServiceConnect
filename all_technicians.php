...
<div class="app">
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <a class="active" href="all_technicians.php"><i class="fa-solid fa-users"></i> All Technicians</a>
      <a href="all_customers.php"><i class="fa-solid fa-user"></i> All Customers</a>
  ...
  <main class="main">
    <div class="topbar">
      <div>
        <div class="breadcrumb"><a href="admin.php" style="text-decoration:none">Home</a> / All Technicians</div>
    ...
    <div class="card">
      <form class="filters" method="get" action="">
        ...
        <?php if ($q!=='' || $status!==''): ?>
          <a class="btn secondary" href="all_technicians.php">Reset</a>
        <?php endif; ?>
      </form>
      ...
      <table class="table">
        <thead>
          <tr>
            <th>#</th><th>Name</th><th>Email</th><th>Phone</th><th>Status</th><th>Joined</th>
          </tr>
        </thead>
        <tbody>
          <?php if ($techs->num_rows === 0): ?>
            <tr><td colspan="6">No technicians found.</td></tr>
          <?php else: $i = $offset + 1; while($t = $techs->fetch_assoc()):
            $isActive = strtolower($t['status'] ?? '') === 'active'; ?>
            <tr>
              <td><?= $i++ ?></td>
              <td><?= esc(trim(($t['first_name'] ?? '').' '.($t['last_name'] ?? ''))) ?></td>
              <td><?= esc($t['email'] ?? '') ?></td>
              <td><?= esc($t['phone'] ?? '') ?></td>
              <td><span class="pill-status <?= $isActive?'active':'inactive' ?>"><?= esc(ucfirst($t['status'] ?? '')) ?></span></td>
      ...
      <?php if ($total_pages > 1): ?>
        <div class="pager">
          <?php for ($p=1; $p<=$total_pages; $p++):
            ...
            <a class="<?= $p==$page?'current':'' ?>" href="all_technicians.php?<?= $qs ?>"><?= $p ?></a>
          <?php endfor; ?>
        </div>
      <?php endif; ?>
    </div>
