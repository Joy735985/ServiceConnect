<?php
// edit_profile.php — Technician (also works for Customer/Admin)
// PHP 7.3 compatible; includes Change Password + Profile image upload
...
if (function_exists('requireRole')) {
  // Allow Technician; keep Admin/Customer if you reuse this screen
  requireRole(['Technician','Admin','Customer']);
} else {
  $role = isset($_SESSION['user']['role']) ? $_SESSION['user']['role'] : '';
  if (!in_array($role, ['Technician','Admin','Customer'], true)) {
...
$me = isset($_SESSION['user']) ? $_SESSION['user'] : null;
$userId = (int)($me['id'] ?? 0);
if (!$userId) { die('Missing user id in session.'); }
...
// ---- Buckets for messages ----
$errors = array();      // profile update errors
$pwErrors = array();    // password change errors
$pwSuccess = null;      // password change success
  ...
  if (!$pwErrors) {
    ...
    if (!$stmt) {
      ...
      if (!$row) {
        ...
        // Verify current (supports legacy plaintext)
...
// Re-fetch after redirect to display latest values
...
// Determine base role for sidebar and switch logic
$baseRole = strtolower($me['role'] ?? 'technician'); // Default to technician if role is unset
...
if ($userId) {
    ...
    if ($curEmail) {
        $targetRole = ($baseRole === 'technician') ? 'customer' : 'technician';
        
        $stmt = $conn->prepare("SELECT id FROM users WHERE email = ? AND LOWER(role) = ? LIMIT 1");
        $stmt->bind_param("ss", $curEmail, $targetRole);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0) {
          $text = ($baseRole === 'technician') ? 'Customer' : 'Technician';
          $switchProfileLink = '<a href="' . $BASE . 'switch_profile.php?to=' . $targetRole . '" class="sidebar-link">🔁 Switch to ' . $text . ' Profile</a>';
        } else {
          $text = ($baseRole === 'technician') ? 'Customer' : 'Technician';
          $switchProfileLink = '<a href="' . $BASE . 'signup.php?role=' . $targetRole . '" class="sidebar-link">➕ Create ' . $text . ' Account</a>';
        }
        $stmt->close();
    }
}
...
<head>
  ...
  <style>
    ...
    @media (max-width: 768px){
      ...
      .toprow h2, .toprow h3{font-size:22px;}
    }
...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="toprow">
      <h2>Edit Profile</h2>
      <div class="pill">Account Settings</div>
    </div>
